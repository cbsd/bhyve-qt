...
//    m_osListWidget->setMaximumWidth(170);
    m_osListWidget->setMaximumWidth(240);
...
// Machine::getCPUCount  cpu["CPUCount"]    = this->CPUCount;

//    m_machineDetailsLayout->addRow(tr("CPU") + ":",m_machineCPUCount);
...
void MainWindow::createMenusActions()
{
    ...
    // VNC
    m_pauseMachineAction = new QAction(this);
    m_pauseMachineAction->setIcon(QIcon::fromTheme("project-development-new-template",
                                                   QIcon(QPixmap(":/images/icons/breeze/32x32/project-development-new-template.svg"))));
    m_pauseMachineAction->setToolTip(tr("VNC"));
    connect(m_startMachineAction, &QAction::triggered,
            this, &MainWindow::runMachine);
//            this, &MainWindow::vncMachine);
}
...
//    QString qtemuConfig = dataDirectoryPath.append("qtemu.json");
    QFile machinesFile("/usr/jails/tmp/bhyve-vm.json");
...
//        this->generateMachineObject(machines[i].toObject(), i);
    ...
    printf("LOADED: %d\n",machines.size());
//    SystemUtils::showMessage("QtEmu - Profiles loaded","Profiles loaded", QMessageBox::Critical);
    return;
...
void MainWindow::generateMachineObject(const QJsonObject machineConfigJsonObject, int pos)
{
    ...
    QString jname_status;
    QString jname=machineJSON["name"].toString();
    QString fileName("/dev/vmm/" + jname);
    QFile file(fileName);
    int status;

if(QFileInfo::exists(fileName))
{
    jname_status=( jname + "(ON)" );
    file.close();
    status=0;
}
else
{
    ...
    jname_status=( jname + "(OFF)" );
    status=0;
}

    QListWidgetItem *machineListItem = new QListWidgetItem( jname_status , this->m_osListWidget);
    ...
    machine->setState(Machine::Started);
...
//    this->m_machineCPULabel->setText(machine->getCPUType());
    this->m_machineCPULabel->setText(QString::number(machine->getCPUCount()));
