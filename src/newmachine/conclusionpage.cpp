...
bool MachineConclusionPage::validatePage()
{
    ...
    QString vm_ram = QString::number(this->m_newMachine->getRAM());
    QString imgsize = QString::number(diskSize);


//LOAD profles
    QFile file;
    file.setFileName("/usr/jails/tmp/bhyve-vm.json");
    ...
    foreach(QVariant value, list)
    {
        ...
        curName = map["name"].toString();
        curProfile = map["profile"].toString();
        found=0;
        if ( this->m_newMachine->getOSVersion() == curName ) {
		    this->m_newMachine->setOSProfile(curProfile);
                    found=1;
                    break;
        }
    }
    ...
    if ( found == 0 ) {
	SystemUtils::showMessage(tr("Bhyve-QT - Error Out"), "Unable to map name -> profile",QMessageBox::Critical);
    }

//this->m_newMachine->getRAM()).append(" MiB")
//    if (createNewDisk) {
        QString diskPathName = "/usr/local/bin/sudo /usr/local/bin/cbsd";
        diskPathName.append(" bcreate jname=")
                    .append(this->m_newMachine->getName())
                    .append(" vm_ram=").append(vm_ram).append("m")
                    .append(" imgsize=").append(imgsize).append("g")
                    .append(" vm_cpus=1")
                    .append(" vm_os_type=").append(this->m_newMachine->getOSType())
//                    .append(" vm_os_profile=").append(this->m_newMachine->getOSVersion());
                    .append(" vm_os_profile=").append(this->m_newMachine->getOSProfile());
//    }

//                    .append("m"))

//    SystemUtils::showMessage(tr("Bhyve-QT - Debug"),
//                             diskPathName,
//                             QMessageBox::Critical);


    QProcess proc;
      proc.start(diskPathName);
//    proc.start("echo " + jname);
...
//QString bcreateCmd;
// = "/usr/local/bin/cbsd bcreate";
//bcreateCmd.append(bcreateCmd," jname=xxx");
...
void MachineConclusionPage::insertVMList()
{
    ...
    machine->setIcon(QIcon(":/images/os/64x64/freebsd.png"));
