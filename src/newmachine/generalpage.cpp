                                 ...
                                 QWidget *parent) : QWizardPage(parent)
{
    ...
    m_OSType = new QComboBox(this);
						// enum:
//    m_OSType->addItem("freebsd");		// 1
//    m_OSType->addItem("linux");			// 2
//    m_OSType->addItem("openbsd");		// 3
//    m_OSType->addItem("netbsd");		// 4
//    m_OSType->addItem("dflybsd");		// 5
//    m_OSType->addItem("windows");		// 6
//    m_OSType->addItem("other");			// 7
...
//LOAD profles
    ...
    foreach(QVariant value, list)
    {
        ...
        curType = map["type"].toString();
        found=0;
        for (int index = 0; index < m_OSType->count(); index++)
            if ( m_OSType->itemText(index) == curType ) {
                    found=1;
                    break;
            }
        if (found==0)
            m_OSType->addItem(curType);
    }
...
//    for (int index = 0; index < m_OSType->count(); index++) {
//        this->m_OSVersion->addItem(tr("FreeBSD 13.2"));
...
//        this->m_OSVersion->addItem(curName);
...
/*
    if (OSSelected == 0) {
        this->m_OSVersion->addItem(tr("FreeBSD 13.2"));
        this->m_OSVersion->addItem(tr("FreeBSD 13.2 cloud"));
    } else if (OSSelected == 1) {
        this->m_OSVersion->addItem(tr("Debian 11"));
        this->m_OSVersion->addItem(tr("Debian 11 cloud"));
    } else if (OSSelected == 2) {
        this->m_OSVersion->addItem(tr("OpenBSD 7"));
        this->m_OSVersion->addItem(tr("OpenBSD 7 cloud"));
...
bool MachineNamePage::validatePage()
{
    ...
    if (QDir(fullMachinePath).exists()) {
        ...
        m_createMachineMessageBox->setWindowIcon(QIcon::fromTheme("CBSD", QIcon(":/images/qtemu.png")));
