...
bool MachineUtils::deleteMachine(const QUuid machineUuid)
{
    ...
    while(machinePos < machines.size() && ! machineExists) {
        ...
        if (machineUuid == machineJSON["uuid"].toVariant()) {
            ...
            machineJname = machineJSON["jname"].toString();
        } else {
            ++machinePos;
        }
    }
...
//        machines.removeAt(machinePos);
    SystemUtils::showMessage(tr("QEMU - Error Out"),
                             "/usr/local/bin/sudo /usr/local/bin/cbsd bremove jname="+machineJname,
                             QMessageBox::Critical);

  QProcess proc;
  proc.start("/usr/local/bin/sudo /usr/home/oleg/.qtemu/run.sh > /tmp/new.json 2>&1");
