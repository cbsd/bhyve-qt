...
QString Machine::getOSProfile() const
{
    return OSProfile;
}
...
void Machine::setOSProfile(const QString &value)
{
    OSProfile = value;
}
...
//    program.append(QEMUGlobalObject->getQEMUBinary("/usr/local/bin/sudo /usr/local/bin/cbsd"));
    program.append("/usr/local/bin/sudo");
//    program.append("/usr/local/bin/cbsd");
//    program.append(QEMUGlobalObject->getQEMUBinary("/usr/local/bin/sudo /usr/local/bin/cbsd"));
...
//  process.start("kwrite");

    // Execute shell command
//    proc.start("/usr/local/bin/kwrite");
...
QString jname = this->name;
QString vncport;
//    SystemUtils::showMessage(tr("QEMU - Error Out"),
//                             jname,
//                             QMessageBox::Critical);

QString fileName("/usr/jails/jails-system/" + jname + "/vnc_port");
QFile file(fileName);
if(QFileInfo::exists(fileName))
{
    file.open(QIODevice::ReadOnly | QIODevice::Text);
    vncport =  file.readAll();
    file.close();
}
else
{
...
//  process.start("kwrite");
    ...
    // Execute shell command
    proc.start("/usr/bin/env VNC_PASSWORD=cbsd /usr/local/bin/vncviewer 127.0.0.1:" + vncport);
...
//        return "";
    ...
    SystemUtils::showMessage(tr("no such vnc_port"),
                             fileName,
                             QMessageBox::Critical);
...
//    qemuCommand << "-name";
    qemuCommand << "/usr/local/bin/cbsd";
    qemuCommand << "bstart";
    qemuCommand << "jname=" << this->name;
    qemuCommand << "inter=0";
...
bool Machine::saveMachine()
{
    ...
    machineJSONObject["OSProfile"]   = this->OSProfile;
